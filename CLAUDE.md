...
- **`slides.md`** — the deck entry, deliberately thin: the first frontmatter block is the deck config (headmatter), which doubles as the title slide's frontmatter, so the title slide and its `ExponentialSpiral` background live here. Below it, one bare `src: ./pages/NN-<section>.md` import stub per workshop section, in agenda order. Keep stubs bare (only `src:`) — on Slidev's frontmatter merge the entry's keys override the imported file's first slide, so any extra key here would silently shadow the section file.
- **`pages/NN-<section>.md`** — one file per agenda section (numbered to match `references/agenda.md`), holding all of that section's slides separated by `---`. Each slide carries its own frontmatter (`kicker`, `layout`, …); speaker notes go in an HTML comment `<!-- ... -->` at the end of each slide. Slides use Vue components, UnoCSS utility classes, and Slidev click animations (`v-click`, `v-mark.underline/circle/highlight.<color>` with `{ at: N }` for click timing). Unwritten sections hold a placeholder slide whose speaker note carries the section's agenda beats as a TODO — replace the placeholder when writing the section.
- **`components/Rough.vue`** — auto-registered by Slidev. Renders self-drawing sketchy SVG shapes via rough.js (`shape`: `rect | ellipse | line | arrow | curve`; `curve` draws a smooth line through the `points` prop, `[[x, y], …]` in local px). Key props: `color`, `fill`, `fillStyle` (`hachure`, `cross-hatch`, `dots`, `scribble`...), `delay` (ms, for staggering multiple shapes), `seed` (changes the wobble), `at` (reveal on click N — the slide needs that many clicks registered, e.g. via `v-click` on text). Slot content is centered inside the shape. Draw-in animation only runs in `slide`/`presenter` render contexts; in PDF/PPTX exports shapes render fully drawn.
- **`components/ExponentialSpiral.vue`** — auto-registered full-bleed WebGL background for the title slide. It renders the recursive exponential spiral, pauses outside the active slide, respects reduced motion, and uses a deterministic still frame for print/export contexts.
- **`uno.config.ts`** — design tokens and shortcuts, merged into Slidev's built-in UnoCSS (preset-wind3) config. Defines the `theme.colors` palette (`paper`/`paper-deep`, `ink`/`ink-soft`, `accent`/`accent-soft`, `green`/`green-deep`, `line`) used as Tailwind-style utilities (`bg-paper`, `text-ink-soft`, `text-accent`, `stroke-line`, …), the `bg-dotted-grid` custom rule (Excalidraw-style dotted paper background), and the `hand` shortcut (italic Fraunces annotation voice).
...
- **`global-top.vue`** — renders the editorial frame (kicker top-left + `§N` slide number top-right, styled with inline utility classes) on any slide that declares `kicker: <topic>` in its frontmatter. The title slide omits it, and swaps the dotted grid for the full-bleed `ExponentialSpiral` canvas (`class: title-slide`).
...
- **Motion**: ease with `cubic-bezier(0.22, 1, 0.36, 1)`, 0.5–0.65s, elements rising ~14px into place; connectors draw in via stroke-dashoffset; stagger sequential elements rather than revealing them at once. Respect `prefers-reduced-motion` in any hand-written animation (`Rough.vue` and `ExponentialSpiral.vue` already handle exports).

## Voice & copy

Applies to all deck text: slide copy, speaker notes, and diagram labels.

- **Professional but friendly.** Write like a knowledgeable mentor talking to the audience: warm, direct, and plainspoken. Confident without being stiff or corporate.
- **Never catchy.** No marketing-speak, hype, clickbait phrasing, or slogan-style copy. Avoid buzzwords, superlatives ("amazing", "game-changer", "revolusioner"), and exclamation marks used for excitement. Let the content carry the energy, not the punctuation.
- **No em dashes.** Do not use the em dash (—) anywhere in deck text. Restructure the sentence, or use a comma, colon, or parentheses instead. (This rule is for deck content; code comments and this file are exempt.)
...
- `.hand` is an `uno.config.ts` shortcut (Tailwind classes under the hood), not a hand-written CSS class — use it the same way as any utility class.
- For the rotated "sticker" feel, use the `-rotate-2` / `rotate-2` utilities directly instead of the old `.tilt-l`/`.tilt-r` classes.
- When adding shapes to a diagram, stagger `delay` values so shapes draw in sequence (see the flow diagram slide in `pages/02-what-is-vibe-coding.md`).
...
- **Editing sections (parallel-agent friendly)**: to change a section's slides, edit only its `pages/NN-<section>.md` file — never `slides.md`. This keeps concurrent edits to different sections conflict-free, so separate agents can each own a section. Touch `slides.md` only to add, reorder, or remove whole sections (new section = new `pages/` file + one bare `src:` stub in `slides.md`). Slide numbers (`§N` in the header frame) are computed globally by Slidev after imports resolve, so files never need renumbering.
